...
def num_below_thousand(num):
    ...
    if num < 20:  # anything under 20 has its own number
        ...
        if end:
            join = '-'
        return (nums[num // 10 * 10] + join + end).strip()
    elif num < kilo:
...
def num_to_words(num, scale=1):
    ...
    if num < 1000:
        ...
        join = ''
        return (num_below_thousand(num) + ' ' + p + join).strip()
    else:
        scale *= 1000
        top, bot = divmod(num, 1000)
        result = num_to_words(top, scale)
        last = num_to_words(bot, scale // 1000) if bot else ''
        join = ''
        if bot:
            if bot < 100:
                join = ' and '
